...
if C.mode == 'train':
    ...
    model = fetch(C, ckpt=C.ckpt, strict=True, dp=True)
elif C['mode'] == 'val':
    model = fetch(C, ckpt=C.ckpt, strict=True, dp=True)
else:
        ...
        if epoch>=0:
            ...
            with torch.no_grad():
                ...
                for val_data in tqdm(test_loader):
                    step +=1
                    fea, val_labels, id, hos = val_data
                    fea, val_labels  = fea.to(device), val_labels.to(device)

                    hos_all.extend(hos)
                    val_outputs = model(fea)
