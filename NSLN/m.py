...
class XNet(nn.Module):
    def __init__(self,a):
        ...
        self.classification_layers = nn.Sequential(nn.Linear(2048*2, 512), nn.ReLU6(), nn.Linear(512, 2))
        self.a = a
        if a:
            self.attmil = AttMIL(in_features=1024, out_features=1024)
    def forward(self, x1, x2, f):        
        if self.a:
            return self.classification_layers(torch.cat([self.attmil(self.encoder_tumor(x1)), self.encoder_axillary(x2), fea], dim=1))
        else:
            return self.classification_layers(torch.cat([self.encoder_tumor(x1), self.encoder_axillary(x2), fea], dim=1))
