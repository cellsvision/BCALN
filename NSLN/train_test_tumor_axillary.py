...
if C.mode == 'train':
    ...
    model = fetch(ckpt=C.ckpt, strict=True, dp=True)
elif C['mode'] == 'val':
    model = fetch(ckpt=C.ckpt, strict=True, dp=True)
else:
...
if (C.mode == 'train') or (C.mode == 'continue_train'):
    ...
    for epoch in range(C.eps):
        ...
        for batch_data in train_loader:
            ...
            step += 1
            tumor, axillary, labels, fea = (
                batch_data["tumor"].to(device),
                batch_data["axillary"].to(device),
                batch_data["gt"].to(device),
                batch_data["fea"].to(device),
            )
            optimizer.zero_grad()
            outputs = model(tumor, axillary, fea)
        ...
        if epoch>=0:
            ...
            with torch.no_grad():
                ...
                for val_data in tqdm(test_loader):
                    ...
                    tumor, axillary, val_labels, hos, fea = (
                        val_data["tumor"].to(device),
                        val_data["axillary"].to(device),
                        val_data["gt"].to(device),
                        val_data["hos"],
                        val_data["fea"].to(device),
                    )
                    hos_all.extend(hos)
                    val_outputs = model(tumor, axillary, fea)
    ...
    with torch.no_grad():
        step = 0
        for val_data in tqdm(test_loader):
            tumor, axillary, val_labels, hos, ID, fea = (
                val_data["tumor"].to(device),
                val_data["axillary"].to(device),
                val_data["gt"].to(device),
                val_data["hos"],
